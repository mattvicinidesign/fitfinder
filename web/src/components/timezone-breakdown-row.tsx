...
export function TimezoneBreakdownRow({
  ...
  const detail = buildTimezoneDetail(
    parsedJob,
    parsedResume,
    profileTimezone,
    category,
      ...
      ? [
          {
            ...
            resumeMatch: detail.candidateTimezone ?? undefined,
          },
        ]
      : [];
  ...
  const missingItems =
    detail.outcome === "mismatched" && detail.jobRequirement
      ? [
          {
            label: detail.jobRequirement,
            subtext: detail.candidateTimezone
              ? `Your timezone (${detail.candidateTimezone}) does not overlap this requirement after normalization`
              : null,
          },
        ]
      : [];
    ...
    detail.outcome === "no_job_requirement" ? (
      <>
        ...
        <p className="text-[11px] text-muted-foreground">
          The job parser did not extract timezoneRequirement — this row stays Unknown
  ...
  return (
    <BreakdownAccordion
      summary={summary}
      ariaLabel="Timezone match breakdown"
      expandHint="timezone details"
    >
      <div className="space-y-3 text-[13px] leading-snug">
        ...
        <div className="space-y-1 border-t border-border/60 pt-2">
          ...
          <p className="text-[15px] font-medium text-foreground leading-snug">
            {detail.candidateTimezone ?? "Not detected"}
          </p>
          {detail.candidateSource === "resume" ? (
            ...
            <p className="text-[11px] text-muted-foreground">
              From Profile (resume parse omitted timezone)
            </p>
          ) : detail.outcome === "no_candidate_timezone" && detail.jobRequirement ? (
            <p className="text-[11px] text-muted-foreground">
