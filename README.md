...
After **any** `web/` UI change, run `cd web && npm run cap:sync` before testing in Xcode, then commit `build-meta.json` and push to `main` for Vercel. The home search bar badge shows **v0.1.N** from `web/build-meta.json` (incremented each cap sync).
...
**Home & Stats activity:** Recent activity uses one shared list UI on Home (max 10 rows + **View All** → `/stats#all-activity`) and Stats (**All Activity**, lazy-load +20). Rows show a **Fit Analysis** or **Resume Score** pill and fit scores as `6.6/10` (smaller `/10` denominator via `FitScoreRatio`). Resume score rows link to `/resume-review?reviewId=…` and reopen that specific report from `localStorage` history (`fitfinder:resume-review:history`). Fit analysis reports are mirrored to **localStorage** (not just `sessionStorage`) so recent activity survives iOS hard quit. Empty states use a shared zero illustration; Splash QA **Hard refresh** / **Reset first launch** force empty activity **and** zeroed Stats KPIs (no sample fallback).

**Home hero:** Always shows the blue **Avg Fit Score** card (not the welcome headline) once loaded. Average is computed from real fit analyses in activity only — **0.0** when none. Subtitle is **Last activity:** with relative time (`11 minutes ago`, or **0 days ago** when empty). Never uses sample analyses for the hero average.

**Stats dashboard:** Hero KPIs — **Total Fit Analyses**, **Total Resume Scores**, **Avg Fit Analysis**, **Avg Resume Score** — are computed from real activity via `pickFitAnalysesForMetrics` (not sample padding when lists are empty). Fit averages use the same rollup engine as the report summary ring (`web/src/lib/report-display-score.ts` → `resolveReportFitScoreFromCacheEntry`), so Home, Stats, and the fit report always show the same global score for a given analysis. **Avg Fit Analysis by Category** averages Overall Match rollups — About Client, Role Alignment, Qualifications, Preferences — across cached fit reports (`computeOverallMatchCategoryAverages`).
