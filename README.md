...
**Stats dashboard:** Hero KPIs — **Total Fit Analyses**, **Total Resume Scores**, **Avg Fit Analysis**, **Avg Resume Score** — use the same real recent-activity rows as Home (no sample padding; guest first install shows **0** / **—**). Fit averages use the same rollup engine as the report summary ring (`web/src/lib/report-display-score.ts` → `resolveReportFitScoreFromCacheEntry`), so Home, Stats, and the fit report always show the same global score for a given analysis. **Avg Fit Analysis by Category** averages Overall Match rollups — About Client, Role Alignment, Qualifications, Preferences — across cached fit reports (`computeOverallMatchCategoryAverages`).
