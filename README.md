...
**Home & Stats activity:** Recent activity uses one shared list UI on Home (max 10 rows + **View All** → `/stats#all-activity`) and Stats (**All Activity**, lazy-load +20). Rows show a **Fit Analysis** or **Resume Score** pill and fit scores as `6.6/10` (smaller `/10` denominator via `FitScoreRatio`). Empty states use a shared zero illustration; Splash QA **Hard refresh** / **Reset first launch** can force empty lists for QA without re-seeding sample rows.

**Stats dashboard:** Hero KPIs — **Total Fit Analyses**, **Total Resume Scores**, **Avg Fit Analysis**, **Avg Resume Score** — are computed from real activity (not sample padding). Fit averages use the same rollup engine as the report summary ring (`web/src/lib/report-display-score.ts` → `resolveReportFitScoreFromCacheEntry`), so Home, Stats, and the fit report always show the same global score for a given analysis.
