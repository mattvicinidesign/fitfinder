...
const UPWORK_SKILLS_SECTION_START =
  /^(?:#{1,3}\s*)?skills?\s+and\s+expertise\s*$/i;

const UPWORK_MANDATORY_SKILLS_HEADER =
  /^(?:#{1,3}\s*)?(?:mandatory\s+skills?|required\s+skills?)\s*[:.]?\s*(.*)$/i;

const UPWORK_OPTIONAL_SKILLS_HEADER =
  /^(?:#{1,3}\s*)?(?:nice\s*to\s*have\s+skills?|optional\s+skills?|preferred\s+skills?)\s*[:.]?\s*(.*)$/i;
...
function parseSkillTagLine(line: string): string[] {
  const trimmed = line.trim();
...
function parseInlineSkillTags(rest: string): string[] {
  const trimmed = rest.trim();
  if (!trimmed) return [];
  return parseSkillTagLine(trimmed);
}

/** Upwork "Skills and Expertise" tagged chips — mandatory vs optional. */
export function extractUpworkTaggedSkills(jobText: string): {
  mandatory: string[];
  optional: string[];
} {
  ...
  let inSection = false;
  let mode: "mandatory" | "optional" = "mandatory";

  for (const line of jobText.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed) continue;

    if (UPWORK_SKILLS_SECTION_START.test(trimmed)) {
      inSection = true;
      mode = "mandatory";
      continue;
    }
    ...
    if (optionalMatch) {
      mode = "optional";
      optional.push(...parseInlineSkillTags(optionalMatch[1] ?? ""));
      continue;
    }

    const tags = parseSkillTagLine(trimmed);
    if (tags.length === 0) continue;
    if (mode === "optional") optional.push(...tags);
    else mandatory.push(...tags);
  }

  return {
    mandatory: dedupeLabels(mandatory),
    optional: dedupeLabels(optional),
  };
}
...
export function normalizeParsedJob(
  ...
  const { skills: modelSkills, toolsFromSkills } = partitionSkills(
    parsed.skills ?? [],
  );
  ...
  const taggedMandatorySkills = dedupeLabels(
    partitionSkillsAndTools([
      ...taggedSkills.mandatory,
      ...taggedSkills.optional,
    ]).skills,
  );

  let skills: string[];
  if (taggedMandatorySkills.length > 0) {
    // Upwork tagged skills trump LLM body mining — avoids 0/N on irrelevant body terms.
    skills = taggedMandatorySkills;
  } else {
    skills = dedupeLabels([...modelSkills, ...fromModel.rehomedAsSkills]);
  }
